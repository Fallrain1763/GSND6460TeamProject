...
public class SpellSlot : MonoBehaviour, IPointerDownHandler
{
    ...
    public void Setup(SpellData data, SlotSource src, int idx, SpellInventoryUI mgr)
    {
        spell   = data;
        source  = src;
        index   = idx;
        ...
        if (label != null)
        {
            if (source == SlotSource.Hotbar)
                ...
                label.text = hasSome ? spell.displayName : "";
        }
    }

    public void SetupForge(SpellData data, SlotSource src, int idx, SpellInventoryUI mgr, Image slotImage)
    {
        spell   = data;
        source  = src;
        index   = idx;
